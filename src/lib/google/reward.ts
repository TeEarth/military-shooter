...
export async function claimMail(playerId: string, index: number): Promise<{ coin: number; exp: number; ticket: number; itemGranted?: string }> {
  ...
  const result = { coin: 0, exp: 0, ticket: 0, itemGranted: undefined as string | undefined };
