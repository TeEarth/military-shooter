...
import type { PlayerIncomeRow, WithdrawalRequestRow } from "@/lib/google/income";
...
export default function IncomeClient({ income: initialIncome, requests: initialRequests, topUpPackages, ticket: initialTicket }: Props) {
  ...
  async function withdraw() {
    ...
    if (loading || !amount || amount <= 0) return;
    sfx.play("ui_click");
    setLoading(true);
    setMessage("");
    try {
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="card-military">
          ...
          <p className="text-xs text-military-steel mb-3">
            Earned from boss kills and every 5 personal stage-milestones. 1 banknote = ฿1. Withdrawals are reviewed and paid out manually by an admin — this is not an automatic cash-out.
          </p>
          ...
          <div className="flex gap-2 mb-3">
            <input
              type="number"
              min={1}
              max={income.greenBanknoteBalance}
              value={withdrawAmount}
              onChange={(e) => setWithdrawAmount(e.target.value)}
              placeholder="Amount"
              className="bg-military-darker border border-military-steel px-2 py-1 text-sm w-32"
            />
            <button onClick={withdraw} disabled={loading || income.greenBanknoteBalance <= 0} className="btn-gold text-xs px-4">
              Request Withdrawal
            </button>
          </div>
            ...
            <div className="space-y-1">
              ...
              {requests.map((r) => (
                <div key={r.id} className="flex justify-between text-xs">
                  ...
                  <span>{r.amount} 💵</span>
                  <span className={r.status === "paid" ? "text-green-400" : "text-military-gold"}>{r.status.toUpperCase()}</span>
                </div>
              ))}
            </div>
          )}
